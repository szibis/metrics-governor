...
sidebar_position: 1
description: "Adaptive limiting with tiered escalation"
---
...
Limits are configured via a YAML file specified with `-limits-config`. See [examples/limits.yaml](https://github.com/szibis/metrics-governor/blob/main/examples/limits.yaml) for a complete example.
...
For full details on each tracking mode, see **[Cardinality Tracking](/docs/governance/cardinality-tracking)**.
...
For full details on the reload mechanism, the configmap-reload sidecar, Helm chart setup, monitoring metrics, and what settings require a full restart, see **[Dynamic Configuration Reload](/docs/operations/reload)**.
