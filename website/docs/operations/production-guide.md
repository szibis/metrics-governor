...
sidebar_position: 1
description: "Production deployment best practices and sizing"
---
...
> **Other references**: [Resilience](/docs/operations/resilience) (circuit breaker, backoff, failover queue), [Performance](/docs/advanced/performance) (internals, benchmarks), [Limits](/docs/governance/limits) (rule syntax, adaptive limiting), [Configuration](/docs/getting-started/configuration) (full YAML and CLI reference).
...
> **DaemonSet mode**: For Enterprise tier, consider `kind: daemonset` to run one pod per node, eliminating cross-AZ traffic costs and reducing latency. See [Kubernetes Configuration](#daemonset-mode).
...
See [limits.md](/docs/governance/limits#per-group-stats-threshold) for full configuration.
...
See [Cardinality Tracking](/docs/governance/cardinality-tracking) for full configuration reference and PromQL monitoring queries.
...
For detailed resilience configuration and failure mode analysis, see [Resilience Guide](/docs/operations/resilience).
...
Five critical alerts every production deployment should have. For a complete alerting reference, see [Alerting Guide](/docs/observability/alerting).
