...
title: "Configuration"
sidebar_position: 2
description: "YAML configuration and CLI flags reference"
---

## Table of Contents

- [Supported Backends](#supported-backends)
  - [OTLP Protocol (gRPC or HTTP)](#otlp-protocol-grpc-or-http)
  - [Prometheus Remote Write (PRW)](#prometheus-remote-write-prw)
...
- [CLI Flags](#cli-flags)
  ...
  - [Queue Options (FastQueue)](#queue-options-fastqueue)
  - [PRW Receiver Options](#prw-receiver-options)
  - [PRW Exporter Options](#prw-exporter-options)
  - [PRW Buffer Options](#prw-buffer-options)
  - [PRW Queue Options](#prw-queue-options)
  - [PRW Sharding Options](#prw-sharding-options)
  - [OTLP Queue Options](#otlp-queue-options)
  - [Queue Resilience Options](#queue-resilience-options)
  - [Memory Limit Options](#memory-limit-options)
  - [Sharding Options](#sharding-options)
  - [Always-Queue & Worker Pool Options](#always-queue--worker-pool-options)
  - [Performance Options](#performance-options)
  - [Telemetry Options (OTLP Self-Monitoring)](#telemetry-options-otlp-self-monitoring)
  - [HTTP Client Tuning (Exporter)](#http-client-tuning-exporter)
  - [Compression Options (Exporter)](#compression-options-exporter)
  - [Receiver HTTP Server Tuning](#receiver-http-server-tuning)
  - [PRW Queue Options](#prw-queue-options-1)
  - [PRW Limits Options](#prw-limits-options)
  - [Cardinality Tracking Options](#cardinality-tracking-options)
...
- [Usage Examples](#usage-examples)
  ...
  - [OTLP Exporter - Backend Examples](#otlp-exporter---backend-examples)
  - [Prometheus Remote Write - Backend Examples](#prometheus-remote-write---backend-examples)
  - [Dual Pipeline (OTLP + PRW)](#dual-pipeline-otlp--prw)
  - [Buffering and Performance](#buffering-and-performance)
  - [Limits Enforcement](#limits-enforcement)
...
See [examples/config.yaml](https://github.com/szibis/metrics-governor/blob/main/examples/config.yaml) for a complete example with all options documented.

For Prometheus Remote Write configuration, see [prw.md](/docs/protocols/prw).

For queue resilience features (circuit breaker, exponential backoff, memory limits), see [resilience.md](/docs/operations/resilience).

Additional example configs:
- [examples/config-minimal.yaml](https://github.com/szibis/metrics-governor/blob/main/examples/config-minimal.yaml) - Minimal configuration
- [examples/config-production.yaml](https://github.com/szibis/metrics-governor/blob/main/examples/config-production.yaml) - Production-ready settings
...
The queue uses a high-performance FastQueue implementation inspired by VictoriaMetrics' persistentqueue. It provides metadata-only persistence with in-memory buffering for high throughput. See [resilience.md](/docs/operations/resilience) for circuit breaker and backoff documentation.
...
The PRW queue uses the same high-performance disk-backed `SendQueue` as the OTLP pipeline, providing persistent storage, circuit breaker, exponential backoff, and split-on-error. See [resilience.md](/docs/operations/resilience) for detailed resilience documentation.
...
The queue provides durability for export failures with memory or disk-backed storage. Memory mode (default) is fast with bounded in-memory queue. Disk mode uses a high-performance FastQueue implementation. See [resilience.md](/docs/operations/resilience) for detailed information on circuit breaker, backoff, failover queue, and split-on-error behavior.

| Flag | Default | Description |
|------|---------|-------------|
| `-queue-enabled` | `true` | Enable failover queue (safety net for export failures) |
| `-queue-type` | `memory` | Queue type: `memory` (bounded, fast) or `disk` (FastQueue, durable) |
| `-queue-mode` | `memory` | Queue mode: `memory`, `disk`, or `hybrid` (L1 memory + L2 disk spillover). See [queue.md](/docs/architecture/queue) for details. |
