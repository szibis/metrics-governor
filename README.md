...
| **Inconsistent label naming** across teams | [Transform rules](docs/processing-rules.md) rename, regex-replace, add, remove, keep, or drop labels — normalize conventions in one place |
| **High-cardinality dimensions** from N services | [Aggregate rules](docs/processing-rules.md) with `group_by` reduce cross-series cardinality — e.g. collapse per-pod into per-service |
| **Noisy test/debug metrics** pollute production | [Drop rules](docs/processing-rules.md) with regex matching remove unwanted metrics before they reach storage |
| **Signal loss** from naive downsampling | [10 downsample methods](docs/processing-rules.md) including adaptive CV-based, LTTB, and SDT preserve signal fidelity while reducing volume |
| **No team ownership** of metric rules | [Classify action](docs/processing-rules.md) + [ownership labels](docs/processing-rules.md) derive team, severity, priority — route alerts via Alertmanager by team |
| **Stale rules** accumulate silently | [Dead rule detection](docs/processing-rules.md#dead-rule-detection) tracks last-match time for every processing and limits rule, with alerts for rules that stop matching |
| **Limits too aggressive** — afraid to enable | [Dry-run mode](docs/limits.md) logs what would be dropped without actually dropping — test limits safely in production |
...
| **Unpredictable costs** from runaway services | [Per-group tracking](docs/limits.md) with configurable limits, [Bloom filters](docs/cardinality-tracking.md) (98% less memory), and ownership labels for team routing |
...
flowchart LR
    ...
    classDef backend fill:#27ae60,stroke:#1a6e3e,color:#fff,stroke-width:2px

    style Sources fill:#eaf2f8,stroke:#2980b9,stroke-width:2px,color:#1a5276
    style MG fill:#fdf6e3,stroke:#d4a017,stroke-width:3px,color:#7d6608
    style OTLP fill:#e8f8f5,stroke:#1abc9c,stroke-width:2px,color:#0e6655
    style PRW fill:#fef9e7,stroke:#f39c12,stroke-width:2px,color:#b7770a
    style Backends fill:#eafaf1,stroke:#27ae60,stroke-width:2px,color:#1a6e3e
```

**Two native pipelines. Full feature parity.** Each stage does its job — failed exports retry automatically through the queue with circuit breaker protection, exponential backoff, and split-on-error.

---

## Features

[Receive](#receive--dual-native-protocols) | [Process](#process--unified-rules-engine) | [Control](#control--intelligent-cardinality-governance) | [Export](#export--high-throughput-pipeline) | [Protect](#protect--zero-data-loss-architecture) | [Scale](#scale--horizontal-and-hierarchical) | [Monitor](#monitor--full-observability) | [Deploy](#deploy--production-ready-from-day-one)
...
| Action | What It Does | Chainable? |
|--------|-------------|:----------:|
| **[Sample](docs/processing-rules.md)** | Stochastic reduction (probabilistic or head-N) | Final |
| **[Downsample](docs/processing-rules.md)** | Per-series compression — 10 methods incl. adaptive CV-based, LTTB, SDT | Final |
| **[Aggregate](docs/processing-rules.md)** | Cross-series reduction with `group_by` — avg, sum, p95, stddev, and more | Final |
| **[Transform](docs/processing-rules.md)** | 12 label operations — rename, regex replace, add, remove, keep, drop | Yes |
| **[Classify](docs/processing-rules.md)** | Derive ownership labels (team, severity, priority) from metric metadata | Yes |
| **[Drop](docs/processing-rules.md)** | Unconditional removal | Final |

**Chainable** actions (Transform, Classify) modify the metric and continue to the next rule. **Final** actions process the metric and stop — no further rules apply. Chain transforms before a final action: e.g. Transform → Classify → Aggregate.
...
- **[Filter Persistence](docs/bloom-persistence.md)** — Save/restore Bloom and HyperLogLog state across restarts, eliminating cold-start re-learning
...
Mix and match **queue mode** (`memory`, `disk`, `hybrid`) and **stats level** (`none`, `basic`, `full`) to fit your priorities:

| Priority | Queue Mode | Stats Level | Durability | Observability | Overhead |
|----------|-----------|-------------|:----------:|:-------------:|:--------:|
| **Maximum Throughput** | `memory` | `none` | — | Global totals only | Lowest |
| **Balanced** (default) | `memory` | `basic` | — | Per-metric + per-rule | Low |
| **Full Observability** | `memory` | `full` | — | Full cardinality tracking | Medium |
| **Durable + Observable** | `hybrid` | `basic` | Overflow to disk | Per-metric + per-rule | Medium |
| **Durable + Full** | `hybrid` | `full` | Overflow to disk | Full cardinality tracking | Medium-High |
| **Safety First** | `disk` | `full` | Full crash recovery | Full cardinality tracking | Highest |
| **Durable Proxy** | `disk` | `none` | Full crash recovery | Global totals only | High |

**Queue modes**: `memory` (fastest, no I/O), `hybrid` (in-memory with disk overflow on backpressure), `disk` (all data persisted, survives crashes).
**Stats levels**: `none` (counters only), `basic` (per-metric datapoints + cardinality), `full` (basic + per-label breakdown + rule tracking).
