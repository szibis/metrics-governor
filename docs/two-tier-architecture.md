...
> **Other references**: [Production Guide](production-guide.md) (sizing, tuning), [Sharding](sharding.md) (consistent hashing for Tier 2 backends), [Configuration](configuration.md) (full YAML and CLI reference).
...
For small clusters (under 50 nodes) with low metric volume, a single-tier Deployment or StatefulSet is simpler and sufficient. See the [Production Guide](production-guide.md) for single-tier sizing.
...
Tier 1 processing rules are defined in [`examples/processing-tier1.yaml`](../examples/processing-tier1.yaml). The key sections:
...
Tier 2 processing rules are defined in [`examples/processing-tier2.yaml`](../examples/processing-tier2.yaml). The key sections:
...
- Tier 2 replicas can use [consistent sharding](sharding.md) to distribute load across backends.
- Memory requirements increase with cardinality tracker usage. See [Cardinality Tracking](cardinality-tracking.md) for tuning.
- Enable [persistent queues](queue.md) on Tier 2 for durability during backend outages.
- Start with the lower bound and scale up based on observed resource usage. Use the [Operations Dashboard](dashboards.md) to monitor actual utilization.
...
See [Sharding](sharding.md) for full configuration details.
...
The [Operations Dashboard](dashboards.md) covers all relevant panels. When running two tiers, filter by the `tier` label (set via Helm values or exporter labels) to separate Tier 1 and Tier 2 views. Key dashboard sections:
...
- [`examples/processing-tier1.yaml`](../examples/processing-tier1.yaml) -- Tier 1 DaemonSet processing rules
- [`examples/processing-tier2.yaml`](../examples/processing-tier2.yaml) -- Tier 2 StatefulSet processing rules
- [Production Guide](production-guide.md) -- sizing, tuning, and cost analysis
- [Sharding](sharding.md) -- consistent hashing for Tier 2 backend distribution
- [Dashboards](dashboards.md) -- Grafana dashboards for monitoring both tiers
- [Configuration](configuration.md) -- full YAML and CLI reference
- [Queue](queue.md) -- persistent queue configuration for Tier 2 durability
